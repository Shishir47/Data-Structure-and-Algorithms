 import java.util.*;
public class LinkedListType extends Node {
		private int count=0;
		private Node first=null;
		private Node last=null;
		boolean found=false;
		String find;
		Node f;
		
		public  LinkedListType() {
			count=0;
			first=null;
			last=null;
		}
		public void initalizeList() {
			count=0;
			first=null;
			last=null;	
		}
		public boolean isEmptyList() {
			if(count==0 || first==null || last==null) //First==null; last==null;
			{
			return true;
			}
			else {
				return false;
			}
		}
		public int length() {
			if(!isEmptyList())
			return count;
			else
				return 0;
		}
		public String toString() {
		StringBuilder sb= new StringBuilder();
		for(Node i= first; i!=null; i=i.getLink()) {
			if(i== first) {
				sb.append(i.getInfo());
			}
			else {
				sb.append("," + i.getInfo());
			}
		}
		return sb.toString();
		}
		public Node getFront() {
		if(!isEmptyList()) {
			return first;
		}
		else {
			return null;
		}
		}
		public Node getBack() {
			if(!isEmptyList()) {
				return last;
			}
			else {
				return null;
			}	
		}
		public void insertFirst(int newItem) {
			Node newNode= new Node(newItem,first);
			first= newNode;
			count++;
			if(last==null) {
				last=newNode;
			}
		}
		public void insertLast(int newItem) {
			Node newNode= new Node(newItem,first);
			if(first==null) {
				first=last=newNode;
			}
			else {
				last.setLink(newNode);
				last=newNode;
				count++;
			}
		}
		public void copyList(Node source) {
			Node destination;
			if(!isEmptyList()) {
				initializeList();
				
			}
			if(source.first==null) {
				initalizeList();
			}
			else {
				Node current = source.getFront();
				while(current!=null) {
					insertLast(current.getInfo());
					current= current.getLink();
				}
			}
		}

		public void search(int searchItem) {
			for(Node i=first;i!=null;i=i.getLink()) {
				if(i.getInfo()==searchItem) {
					f=i;
					found= true;
					find="Found it";
					break;
				}
				else {
				found= false;
				find="Not Found";
				}
			}

		}
		public void deleteNode(Node deleteitem) {
			Node p,q,i;
			if(first==null) {
				System.out.println("Cannot Delete from an Empty List");
			}
			else {
				if(first.getInfo()==deleteitem.getInfo()) {
					p=first;
					first=first.getLink();
					count--;
					if(first==null)
						last=null;
				}
				else {
					p=q=first;
					while ( q != null && found ) {
						if(!q.getLink().equals(deleteitem) ) {
							p=q;
							q=p.getLink();
						}
						else
							found=true;
					}
					if(found) {
						if(q==last) {
							last=p;
							p.setLink(null);
						}
					}
				}
			}
		}


		
}
